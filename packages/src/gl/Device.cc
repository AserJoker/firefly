...
void Device::clear(uint32_t mode) { glClear(mode); }
void Device::drawElements(const DRAW_MODE &mode, uint32_t offset,
                          const uint32_t &count) {
  glDrawElements((GLenum)mode, count, GL_UNSIGNED_INT,
                 (void *)(ptrdiff_t)offset);
}
void Device::activeTextureUnit(uint32_t unitId) {
