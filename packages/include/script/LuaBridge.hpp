...
private:
  ...
  static int luaObjectGet(lua_State *state) {
    ...
    if (lua_type(state, key_idx) == LUA_TSTRING) {
      ...
      auto field = self->getIndex(ctx, lua_tonumber(state, key_idx));
      bridge->dump(state, field);
    }
    return 1;
  }
  static int luaObjectSet(lua_State *state) {
    ...
    if (lua_type(state, key_idx) == LUA_TSTRING) {
      ...
      self->setIndex(ctx, lua_tonumber(state, key_idx), value);
    }
    return 0;
  }
  ...
  static Value::Stack objectGet(core::AutoPtr<Context> ctx, Value::Stack args) {
    ...
    if (key->getType(ctx) == Atom::Type::STRING) {
      ...
      lua_geti(state, obj, key->toNumber(ctx));
    }
  ...
  static Value::Stack objectSet(core::AutoPtr<Context> ctx, Value::Stack args) {
    ...
    if (key->getType(ctx) == Atom::Type::STRING) {
      ...
      lua_seti(state, obj, key->toNumber(ctx));
    }
    lua_settop(state, top);
    return {};
  }
...
private:
  ...
  core::AutoPtr<Value> getObjectMetadata() {
    ...
    meta->setField(ctx, "get",
                   ctx->createValue()->setFunction(ctx, &objectGet));
    meta->setField(ctx, "set",
                   ctx->createValue()->setFunction(ctx, &objectSet));
    return meta;
  }
  core::AutoPtr<Value> getFunctionMetadata() {
    ...
    meta->setField(ctx, "call",
                   ctx->createValue()->setFunction(ctx, &funcCall));
    meta->setField(
        ctx, "typeOf",
  ...
  void dump(lua_State *state, core::AutoPtr<Value> value) {
    ...
    if (objects->getType(ctx) == Atom::Type::NIL) {
      ctx->getNativeGlobal()->setField(ctx, "$objects",
                                       ctx->createValue()->setArray(ctx));
      objects = ctx->getNativeGlobal()->getField(ctx, "$objects");
    }
    if (functions->getType(ctx) == Atom::Type::NIL) {
      ctx->getNativeGlobal()->setField(ctx, "$functions",
                                       ctx->createValue()->setArray(ctx));
      functions = ctx->getNativeGlobal()->getField(ctx, "$functions");
    }
