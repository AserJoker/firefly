...
namespace firefly::runtime {
    ...
    public:
        ...
        void define(
                ...
                std::string alias = "",
                std::string description = "") {
            Param param = {name, alias, [&](const std::string &source) -> void {
                value = source;
            }, description};
            _params.push_back(param);
        }
        std::unordered_map<std::string,std::string> parse(const std::vector<std::string>& args);
        std::string help();
        inline const auto& parsed(){
            return _parsed;
        }
    public:
