...
namespace firefly::core {
    enum class CoPromiseType {
        FULFILLED, REJECTED, PENDING
    };

    template<class T>
    class CoFuture : public Object {
    private:
    ...
    public:
        ...
        [[nodiscard]] const CoPromiseType &status() const { return _status; }

        const T &value() const { return _value; }

        [[nodiscard]] const std::exception &error() const { return _error; }
    };

    template<>
    class CoFuture<void> : public Object {
    private:
        std::exception _error;
        CoPromiseType _status;
...
#ifndef __unix__
            current->fiber = ConvertThreadToFiber(nullptr);
#endif
        }
    ...
    public:
...
#else
            routine->fiber =
                    CreateFiber(STACK_SIZE, (LPFIBER_START_ROUTINE) schedule, nullptr);
#endif
        ...
        static CoPromise<void> start(const std::function<void(ARGS...)> &exec,
            ...
            auto *routine = new Coroutine<void>;
            routine->exec = [=]() -> void {
                exec(args...);
        ...
        template<class T>
        static T wait(const CoPromise<T> &promise) {
